...
def main():
	...
	tools = ['blastn','delta-filter','nucmer','show-coords']
	missing_tools = []
	for tool in tools:
	...
	outpath,tempdir = xtrm.dochecks(args)
	
	# Load elements to be screened
	elements = xtrm.importFasta(args.infile)
	
	# If LTR mode, search for terminal repeats on same strand
	if args.findmode == 'LTR':
		segments = xtrm.getLTRs( elements	=	elements,
		...
		xtrm.segWrite(outpath, segs=segments)
	...
	# Optionally construct synthetic MITE from TIRs if discovered
		...
		segments = xtrm.getTIRs( elements	=	elements,
							flankdist	=	args.maxdist,
							minterm		=	args.minterm,
							minseed		=	args.minseed,
							minid		=	args.minid,
