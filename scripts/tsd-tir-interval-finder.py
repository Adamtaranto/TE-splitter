...
from typing import Dict, List, Tuple, Generator, NamedTuple
import argparse
import sys
...
def load_sequence(fasta_path: str, min_length: int) -> Seq:
    ...
    try:
        ...
        raise FileNotFoundError(f"Could not open FASTA file: {fasta_path}")
...
def main():
    ...
    try:
        ...
        if check_interval_overlap(left_interval, right_interval):
            print(
                "Warning: Left and right intervals overlap. "
        ...
        # Store right sequence length for coordinate correction
        # right_seq_length = len(right_seq)
